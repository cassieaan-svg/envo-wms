...
//   { envoRequestId, wmsRequestId?, status, totalAmount?, items?:[{ wmsCommodityId, qtyDispatched }] }
router.post('/status', async (req, res) => {
  try {
    const { envoRequestId, wmsRequestId, status, totalAmount, items } = req.body || {}
    if (!envoRequestId || !status) {
      return res.status(400).json({ success: false, error: 'envoRequestId and status are required' })
    }
    const allowed = ['submitted', 'picking', 'dispatched']
    if (!allowed.includes(status)) {
      return res.status(400).json({ success: false, error: `Unsupported status: ${status}` })
    }
    const updated = await WarehouseRequestService.applyWmsStatus({ envoRequestId, wmsRequestId, status, totalAmount, items })
