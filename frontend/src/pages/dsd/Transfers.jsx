...
export function Transfers() {
  ...
  async function confirmReceipt(record) {
    ...
    setConfirming(true)
    // Server credits the DSD site stock (site parsed from notes) and marks accepted.
    try {
  ...
  async function updateRequestLine(id, field, value) {
    if (field === 'commodity_id') {
      ...
      const dsdRows = await api.stock.dsd.list({ facility_id: fid, dsd_site_name: dsdSiteName, commodity_id: value }).catch(() => [])
