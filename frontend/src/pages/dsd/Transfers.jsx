...
export function Transfers() {
  ...
  useEffect(() => {
    ...
    const channel = sb.channel(`dsd-transfers-${fid}`)
  ...
  async function confirmReceipt(record) {
    ...
    setConfirming(true)
    const dsdSiteNameResolved = record.notes?.match(/\[DSD:\s*([^\]]+)\]/)?.[1]?.trim() || record.receiving_facility_name || ''
    const { data: dsdStk } = await sb.from('dsd_stock').select('id,quantity')
      .eq('facility_id', fid).eq('dsd_site_name', dsdSiteNameResolved).eq('commodity_id', record.commodity_id).maybeSingle()
    if (dsdStk) {
      await sb.from('dsd_stock').update({ quantity: dsdStk.quantity + record.quantity, updated_at: new Date().toISOString() }).eq('id', dsdStk.id)
    } else {
      await sb.from('dsd_stock').insert({ facility_id: fid, dsd_site_name: dsdSiteNameResolved, commodity_id: record.commodity_id, quantity: record.quantity, updated_at: new Date().toISOString() })
    }
  ...
  async function updateRequestLine(id, field, value) {
    if (field === 'commodity_id') {
      if (!dsdSiteName) return
      const { data: dsdStk } = await sb.from('dsd_stock').select('quantity')
        .eq('facility_id', fid).eq('dsd_site_name', dsdSiteName).eq('commodity_id', value).maybeSingle()
  ...
  async function submitRequest(e) {
    ...
    const payload = lines.map(l => {
      ...
      return {
        ...
        receiving_facility_name: dsdSiteName,
        ...
        notes: `[DSD: ${dsdSiteName}] balance:${l.stock_balance} required:${l.stock_required}${reqNotes ? ' ' + reqNotes : ''}`,
      }
    })
    const { data: insertResult, error } = await sb.from('stock_transfer_log').insert(payload).select()
  ...
  function printSlip(t) {
    ...
    const html = `<style>body{font-family:Arial,sans-serif;font-size:11pt;color:#000;margin:0}h2{margin:0 0 2px;font-size:14pt}.sub{font-size:9pt;color:#666;margin-bottom:16px}.grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 24px;margin-bottom:16px}.field label{font-size:7.5pt;text-transform:uppercase;letter-spacing:.07em;color:#888;display:block;margin-bottom:2px}.field span{font-size:11pt;font-weight:600}.full{grid-column:1/-1}hr{border:none;border-top:1px solid #ccc;margin:16px 0}.sig{display:grid;grid-template-columns:1fr 1fr;gap:24px;margin-top:32px}.sig div{border-top:1px solid #000;padding-top:4px;font-size:8pt;color:#555}</style>
      <h2>Redistribution Request Slip</h2><div class="sub">DSD Request &nbsp;·&nbsp; Printed ${printDate}</div><hr/>
      <div class="grid">
        ...
        <div class="field"><label>To</label><span>${dsdSiteName || '—'}</span></div>
