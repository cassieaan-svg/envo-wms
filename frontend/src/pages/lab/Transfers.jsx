  ...
  async function confirmDispatch(t) {
    ...
    try {
      if (pickedLots.length) {
        ...
        // Fall back to the correction the operator just entered, so the note records
        // the real value rather than the gap.
    ...
    setDispatchingId(null); setDispatchApprovedBy(dispatchSigner()); setDispatchCarrier(dispatchSigner()); setDispatchBatch(''); setDispatchQty(1); setDispatchLots([]); setDispatchLoading(false)
  }
                      ...
                      <div className="flex items-start justify-between gap-4 flex-wrap">
                        ...
                        <div className="flex gap-2 items-center flex-wrap">
                          ...
                          {t.status === 'pending' && isSender && !isDispenser && (
                            <>
                                <Button variant="success" size="sm" onClick={() => { setDispatchingId(t.id); setDispatchApprovedBy(dispatchSigner()); setDispatchCarrier(dispatchSigner()); setDispatchQty(t.quantity); setDispatchBatch(''); setDispatchLots([{ id: Date.now(), selected: null, qty: t.quantity, fixExpiry: '', fixBatch: '' }]) }}>Arrange transfer</Button>
                      ...
                      {dispatchingId === t.id && (
                        <div className="mt-3 p-3 bg-white/5 border border-white/10 rounded-lg space-y-3">
                                ...
                                {dispatchLots.map((dl, i) => (
                                  <div key={dl.id} className="space-y-1">
                                    <div className="flex gap-2 items-center">
                                      ...
                                      <BatchSelect facilityId={fid} commodityId={t.commodity_id} locationType={"store"} value={dl.selected?.key || null} onSelect={opt => { const copy = [...dispatchLots]; copy[i] = { ...copy[i], selected: opt, fixExpiry: '', fixBatch: '' }; setDispatchLots(copy) }} className={inputCls} />
                          ...
                          <div className="flex gap-2">
                            ...
                            <Button variant="default" size="sm" onClick={() => { setDispatchingId(null); setDispatchApprovedBy(dispatchSigner()); setDispatchCarrier(dispatchSigner()); setDispatchBatch(''); setDispatchQty(1); setDispatchLots([]) }}>Cancel</Button>
                          </div>
                        </div>
                      )}
                      {acceptingId === t.id && (
