...
export function RequestWarehouse() {
  ...
  function addLine() {
    ...
    setLines(prev => {
      ...
      if (existing) return prev.map(l => l.commodity_id === c.id ? { ...l, quantity: l.quantity + q } : l)
      return [...prev, { commodity_id: c.id, name: c.name, unit_price: Number(c.unit_price), quantity: q }]
    })
    setPickId(''); setQty('')
  }
  ...
  async function submit() {
    ...
    try {
      const created = await api.warehouseRequests.create({
        items: lines.map(l => ({ commodity_id: l.commodity_id, quantity: l.quantity })),
        ...
        <div className="bg-gray-900 border border-white/10 rounded-xl p-4 space-y-4">
          <div className="flex flex-wrap gap-2 items-end">
            ...
            <div className="flex-1 min-w-[220px]">
              ...
              <select value={pickId} onChange={e => setPickId(e.target.value)}
                className="w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2.5 text-sm text-gray-100 focus:outline-none focus:border-blue-500">
                <option value="">Select a commodity… ({pickable.length})</option>
                {pickable.map(c => <option key={c.id} value={c.id}>{c.name} — {naira(c.unit_price)}</option>)}
              </select>
            </div>
            <div className="w-28">
            ...
            <button onClick={addLine} disabled={!pickId || !(parseInt(qty) > 0)}
          ...
          <div className="flex flex-wrap gap-2 items-end justify-between">
            ...
            <div className="w-full sm:w-52">
              ...
              <input value={phone} onChange={e => setPhone(e.target.value)} inputMode="tel" placeholder="08000000000"
                className="w-full bg-white/5 border border-white/15 rounded-lg px-3 py-2.5 text-sm text-gray-100 placeholder:text-gray-600 focus:outline-none focus:border-blue-500" />
            </div>
            ...
            <button onClick={submit} disabled={busy || !lines.length || !name.trim() || !phone.trim()}
